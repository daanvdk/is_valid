...
doesn't cut it, you might want to know why your data is valid or not. For this purpose all predicates include an optional argument `explain`. If you pass 
`explain=True` with the predicate you will get a special `Explanation`-object
as result. All `Explanation` objects have a `valid`, `code`, and `message`
attribute.
```python
>>> explanation = is_list_of_books(books, explain=True)
>>> explanation.valid
True
>>> explanation.message
...
...         print(i, repr(key), repr(subsubexplanation.message))
